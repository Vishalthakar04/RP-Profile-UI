...
// ─────────────────────────────────────────────────────────────────────────────
// 5. POST /rp/auth/logout
// Invalidates session server-side, then client discards tokens
// ─────────────────────────────────────────────────────────────────────────────
export const logoutUser = async () => {
  try {
    const headers  = await getAuthHeaders()
    const response = await fetch('https://rk-mission-be-2.onrender.com/api/rp/auth/logout', {
      method: 'POST',
      headers,
    })
    const json = await response.json()
