...
export const checkInVisit = async (
  ...
  check_in_time: string        // ← add param
) => {
  try {
    ...
    const response = await fetch(`${BASE_URL}/visits`, {
      method: "POST",
      headers,
      body: JSON.stringify({ school_id, check_in_lat, check_in_long, check_in_time }),  // ← add to payload
    });
    ...
    return { success: false, message: error.message || "Check-in failed", data: null };
  }
};
...
// =============================================================================
export const getVisitDetail = async (visitId: string | number) => {
  try {
    const headers = await getAuthHeaders();
...
// =============================================================================
//  4. PUT /rp/visits/:id
//  Update visit (checkout / status / purpose etc.)
...
export const updateVisit = async (
  ...
  try {
    ...
    if (!response.ok || !json.success) {
      throw new Error(json.message || "Failed to update visit");
    }

    return {
      success: true,
      message: json.message || "Visit updated successfully",
      data: json.data,
    };
  } catch (error: any) {
