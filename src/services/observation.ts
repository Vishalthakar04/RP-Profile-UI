...
export const updateImpactSurvey = async (
  visitId: string | number,
  id: string | number,
  payload: { program_id?: string | number; responses?: Record<string, any> }
) => {
  try {
    const headers = await getAuthHeaders();
    const response = await fetch(
...
export type ChecklistData = {
  visit_id: string;
  checklist: {
    principal_verified:    ChecklistEntry;
    headmaster_verified:   ChecklistEntry;
    coordinator_verified:  ChecklistEntry;
    sections_verified:     ChecklistEntry;
    teacher_verified:      ChecklistEntry;
    observation_completed: ChecklistEntry;
    media_uploaded:        ChecklistEntry;
  };
  canFinish: boolean;
};

export const getVisitChecklist = async (visitId: string | number) => {
  try {
    const headers = await getAuthHeaders();
    const response = await fetch(`${BASE_URL}/visits/${visitId}/checklist`, {
      method: "GET",
      headers,
    });
    const json = await response.json();
    if (!response.ok || !json.success)
      throw new Error(json.message || "Failed to fetch checklist");
    return json;
  } catch (error: any) {
    console.error("getVisitChecklist error:", error);
    return { success: false, message: error.message || "Network error", data: null };
  }
};
...
export const updateChecklistItem = async (
  ...
  try {
    ...
    const json = await response.json();
    if (!response.ok || !json.success)
      throw new Error(json.message || "Failed to update checklist item");
    return json;
  } catch (error: any) {
    console.error("updateChecklistItem error:", error);
    return { success: false, message: error.message || "Failed to update checklist item", data: null };
  }
};
...
export const discardVisit = async (visitId: string | number) => {
  try {
    const headers = await getAuthHeaders();
    const response = await fetch(`${BASE_URL}/visits/${visitId}/discard`, {
      method: "DELETE",
      headers,
    });
    const json = await response.json();
    if (!response.ok || !json.success)
      throw new Error(json.message || "Failed to discard visit");
    return json;
  } catch (error: any) {
    console.error("discardVisit error:", error);
    return { success: false, message: error.message || "Failed to discard visit", data: null };
  }
};

export const getAdoptedClassById = async (visitId: string | number, id: string | number) => {
  try {
    const headers = await getAuthHeaders();
    const response = await fetch(`${BASE_URL}/visits/${visitId}/adopted-class/${id}`, { method: 'GET', headers });
    const json = await response.json();
    if (!response.ok || !json.success) throw new Error(json.message || 'Failed to fetch');
    return json;
  } catch (error: any) {
...
export const getEnablingSessionById = async (visitId: string | number, id: string | number) => {
  try {
    const headers = await getAuthHeaders();
    const response = await fetch(`${BASE_URL}/visits/${visitId}/enabling-sessions/${id}`, { method: 'GET', headers });
    const json = await response.json();
    if (!response.ok || !json.success) throw new Error(json.message || 'Failed to fetch');
    return json;
  } catch (error: any) {
...
export const getImpactSurveyById = async (visitId: string | number, id: string | number) => {
  try {
    const headers = await getAuthHeaders();
    const response = await fetch(`${BASE_URL}/visits/${visitId}/impact-surveys/${id}`, { method: 'GET', headers });
    const json = await response.json();
    if (!response.ok || !json.success) throw new Error(json.message || 'Failed to fetch');
    return json;
  } catch (error: any) {
...
export const deleteClassObservation = async (id: string | number) => {
  try {
    const headers = await getAuthHeaders()
    const response = await fetch(`${BASE_URL}/class-observations/${id}`, {
      method: 'DELETE',
    ...
    const json = await response.json()
    if (!response.ok || !json.success)
      throw new Error(json.message || 'Failed to delete class observation')
    return json
  } catch (error: any) {
    console.error('deleteClassObservation error:', error)
    return { success: false, message: error.message || 'Failed to delete', data: null }
  }
}

export const deleteAdoptedClass = async (visitId: string | number, id: string | number) => {
  try {
    const headers = await getAuthHeaders()
    const response = await fetch(`${BASE_URL}/visits/${visitId}/adopted-class/${id}`, {
      method: 'DELETE',
      headers,
    })
    const json = await response.json()
    if (!response.ok || !json.success)
      throw new Error(json.message || 'Failed to delete adopted class')
    return json
  } catch (error: any) {
    console.error('deleteAdoptedClass error:', error)
    return { success: false, message: error.message || 'Failed to delete', data: null }
  }
}

export const deleteEnablingSession = async (visitId: string | number, id: string | number) => {
  try {
    const headers = await getAuthHeaders()
    const response = await fetch(`${BASE_URL}/visits/${visitId}/enabling-sessions/${id}`, {
      method: 'DELETE',
      headers,
    })
    const json = await response.json()
    if (!response.ok || !json.success)
      throw new Error(json.message || 'Failed to delete enabling session')
    return json
  } catch (error: any) {
    console.error('deleteEnablingSession error:', error)
    return { success: false, message: error.message || 'Failed to delete', data: null }
  }
}

export const deleteNoClassObserved = async (visitId: string | number) => {
  try {
    const headers = await getAuthHeaders()
    const response = await fetch(`${BASE_URL}/visits/${visitId}/no-class-observed`, {
      method: 'DELETE',
      headers,
    })
    const json = await response.json()
    if (!response.ok || !json.success)
      throw new Error(json.message || 'Failed to delete no-class record')
    return json
  } catch (error: any) {
    console.error('deleteNoClassObserved error:', error)
    return { success: false, message: error.message || 'Failed to delete', data: null }
  }
}

export const deleteImpactSurvey = async (visitId: string | number, id: string | number) => {
  try {
    const headers = await getAuthHeaders()
    const response = await fetch(`${BASE_URL}/visits/${visitId}/impact-surveys/${id}`, {
      method: 'DELETE',
      headers,
    })
    const json = await response.json()
    if (!response.ok || !json.success)
      throw new Error(json.message || 'Failed to delete impact survey')
    return json
  } catch (error: any) {
    console.error('deleteImpactSurvey error:', error)
    return { success: false, message: error.message || 'Failed to delete', data: null }
  }
}