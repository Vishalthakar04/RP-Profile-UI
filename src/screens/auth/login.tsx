...
export default function Login() {
  ...
  const [checkingToken, setCheckingToken] = useState(true); // 👈 added

  // ✅ Check if already logged in
  useEffect(() => {
    (async () => {
      try {
        const token = await AsyncStorage.getItem("access_token"); // matches key in verify-otp.tsx
        if (token) {
          navigation.replace("Dashboard"); // matches your navigation.replace("Dashboard") in verify-otp
          return;
        }
      } catch (e) {
  ...
  // ✅ Show spinner while checking token — prevents login UI flash
  if (checkingToken) {
    return (
      ...
      >
        ...
        <View>
          ...
          <Text style={styles.appTitle}>
            Ramakrishna Mission Vivekananda Institute of Values, Gurugram
          </Text>
