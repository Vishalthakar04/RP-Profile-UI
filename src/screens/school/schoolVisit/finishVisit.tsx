...
import {
  checkOutVisit,
  updateVisit,      // ✅ PUT /rp/visits/:id  — saves checkout lat/lng/time
  getVisitDetail,   // ✅ GET /rp/visits/:id  — fetches check_in_time for summary
} from '../../../services/visit'
...
export default function FinishVisit() {
  ...
  const { visitId: contextVisitId, clearVisit, setLastScreen } = useVisit()
  const { visitId: routeVisitId } = (route.params ?? {}) as { visitId?: string | number }
  const visitId = routeVisitId ?? contextVisitId

  useEffect(() => {
    setLastScreen({ name: 'FinishVisit', params: { visitId } })
  }, [visitId])
  ...
  // ── Visit detail (check_in_time from server) ──────────────────────────────
  const [checkInTime,    setCheckInTime]    = useState<string | null>(null)  // ISO string from API
  ...
  // ── Fetch visit detail on mount to get real check_in_time ─────────────────
  useEffect(() => {
    if (!visitId) { setDetailLoading(false); return }
    ;(async () => {
      setDetailLoading(true)
      const res = await getVisitDetail(visitId)
      if (res.success && res.data) {
        // Backend field is check_in_time (or created_at as fallback)
        setCheckInTime(res.data.check_in_time ?? res.data.created_at ?? null)
      }
  ...
  if (!visitId) {
    Alert.alert('Visit ID Missing', 'Unable to find your visit ID. Please go back and try again.')
    return
  }
  if (!checkoutLocation) {
  ...
  // ── Step 1: Checkout first (backend marks visit completed) ───────────────
  ...
  // ── Step 2: Now safe to update lat/lng/time — visit is already completed ─
  const updateRes = await updateVisit(visitId, {
    check_out_lat:  checkoutLocation.lat,
    check_out_long: checkoutLocation.lng,
    check_out_time: now.toISOString(),
  })
console.log('[FinishVisit] updateVisit response ←', updateRes)

  if (!updateRes.success) {
    console.warn('[FinishVisit] updateVisit failed:', updateRes.message)
    // Non-fatal — checkout already succeeded, so continue
  }
  ...
  const checkInLabel  = detailLoading ? 'Loading...' : formatTime(checkInTime)
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 80 }}>
        ...
        {/* ── Visit ID missing banner ── */}
        {!visitId && (
          <View style={s.errorBanner}>
            <Ionicons name="alert-circle" size={16} color="#DC2626" />
            <Text style={s.errorBannerText}>
              Visit ID not found. Please go back and restart.
            </Text>
          </View>
        )}
          ...
          <View style={s.summaryRow}>
            <View style={s.summaryLeft}>
              <View style={s.summaryIconBox}>
                <Ionicons name="enter-outline" size={18} color="#F97316" />
              </View>
        ...
        <View style={[s.statusCard, gpsReady ? s.statusCardGreen : s.statusCardGray]}>
          <Ionicons
            name={gpsReady ? 'checkmark-circle' : 'time-outline'}
            size={20}
            color={gpsReady ? '#16A34A' : '#9CA3AF'}
          />
        ...
        <View style={[s.statusCard, checkInTime ? s.statusCardGreen : s.statusCardGray]}>
          <Ionicons
            name={checkInTime ? 'checkmark-circle' : 'time-outline'}
            size={20}
            color={checkInTime ? '#16A34A' : '#9CA3AF'}
          />
          <Text style={[s.statusText, { color: checkInTime ? '#16A34A' : '#9CA3AF' }]}>
            {detailLoading
        ...
        {/* ── Submit button ── */}
        <TouchableOpacity
          style={[s.submitBtn, (!gpsReady || submitting || !visitId) && s.submitBtnDisabled]}
          onPress={handleSubmit}
          disabled={!gpsReady || submitting || !visitId}
          activeOpacity={0.85}
        >
          {submitting ? (
            ...
            <>
              <Text style={[s.submitText, (!gpsReady || !visitId) && { color: '#9CA3AF' }]}>
                {!visitId   ? 'Visit ID Missing'   :
                 !gpsReady  ? 'Waiting for GPS...' :
                              'Confirm & Submit Visit'}
              </Text>
              <Ionicons
                name="cloud-upload-outline"
                size={18}
                color={gpsReady && visitId ? '#fff' : '#9CA3AF'}
                style={{ marginLeft: 8 }}
              />
            </>
          )}
      ...
      <Modal transparent visible={modalVisible} animationType="fade" onRequestClose={() => {}}>
        <View style={s.modalOverlay}>
          <View style={s.modalCard}>
            ...
            {/* ✅ Show real duration in success modal */}
            {checkInTime && checkoutTime && (
              <View style={s.modalDurationRow}>
                <Ionicons name="time-outline" size={16} color="#F97316" />
                <Text style={s.modalDurationText}>
                  Duration: {calcDuration(checkInTime, checkoutTime)}
                </Text>
              </View>
            )}
...
const s = StyleSheet.create({
  ...
  errorBanner: {
    ...
    backgroundColor: '#FEF2F2', marginHorizontal: 16, marginTop: 8,
    padding: 12, borderRadius: 10, borderWidth: 1, borderColor: '#FECACA',
  },
  errorBannerText: { flex: 1, fontSize: 13, color: '#DC2626', fontWeight: '600' },
  ...
  modalDurationRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 10, backgroundColor: '#FFF7ED', paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20 },
  modalDurationText: { fontSize: 13, fontWeight: '700', color: '#F97316' },
